...
export default function ShareModal({ doc, users, currentUserId, onClose, onToast }) {
  ...
  const sharable = users.filter(
    u => u.id !== currentUserId && !currentShares.some(s => s.id === u.id)
  );
  ...
  const handleShare = async () => {
    ...
    try {
      const r = await fetch(`${API}/documents/${doc.id}/share`, {
        ...
        body: JSON.stringify({ ownerId: currentUserId, shareWithId: selectedUserId }),
      });
      const data = await r.json();
      if (!r.ok) throw new Error(data.error);
      onToast(`Shared with ${data.sharedWith.name}`);
      onClose();
    } catch (e) {
  ...
  const handleRevoke = async (shareUserId, userName) => {
    ...
    try {
      const r = await fetch(`${API}/documents/${doc.id}/share/${shareUserId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
          ...
          <div className="shared-list">
            ...
            {currentShares.map(u => (
              <div key={u.id} className="shared-user-row">
                ...
                <button className="revoke-btn" onClick={() => handleRevoke(u.id, u.name)}>
                  Remove
                </button>
              </div>
            ))}
        ...
        {sharable.length > 0 ? (
          ...
          <p style={{ fontSize: 13, color: '#6e7781', marginBottom: 16 }}>
            All available team members already have access.
          </p>
        )}
